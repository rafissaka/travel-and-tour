...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const removeHover = () => {
        if (hoverHandlerRef.current && elementRef.current) {
          elementRef.current.removeEventListener('mouseenter', hoverHandlerRef.current);
      ...
      const armHover = () => {
        if (!triggerOnHover || !elementRef.current) return;
        removeHover();
        const handler = () => {
  ...
  return React.createElement(Tag, { ref: elementRef, className: classes, style: commonStyle }, text);
};
