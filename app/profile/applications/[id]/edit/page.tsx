...
export default function EditApplicationPage() {
  ...
  useEffect(() => {
    const fetchApplication = async () => {
      try {
        ...
        // Update formData with existing data
        setFormData({
          programId: data.programId || '',
          programName: data.programName || '',
          programCountry: data.programCountry || '',
          programUniversity: data.programUniversity || '',
          programStartDate: formatDate(data.programStartDate),
          programEndDate: formatDate(data.programEndDate),
          ...
          passportExpiryDate: formatDate(data.passportExpiryDate),
          passportIssueCountry: data.passportIssueCountry || '',
          previousTravel: data.previousTravel || false,
          previousTravelDetails: data.previousTravelDetails || '',
          ...
          wassceUrl: data.wassceUrl || '',
          medicalResultsUrl: data.medicalResultsUrl || '',
        ...
        if (data.passportCopyUrl) {
          docs.passport = { url: data.passportCopyUrl, publicId: '', type: 'passport', name: 'Passport Copy' };
        }
        if (data.photoUrl) {
          docs.photo = { url: data.photoUrl, publicId: '', type: 'photo', name: 'Passport Photo' };
        }
        if (data.birthCertificateUrl) {
          docs.birthCertificate = { url: data.birthCertificateUrl, publicId: '', type: 'birthCertificate', name: 'Birth Certificate' };
        }
        if (data.transcriptUrl) {
          docs.transcript = { url: data.transcriptUrl, publicId: '', type: 'transcript', name: 'Transcripts' };
        }
        if (data.wassceUrl) {
          docs.wassce = { url: data.wassceUrl, publicId: '', type: 'wassce', name: 'WASSCE Results' };
        }
        if (data.medicalResultsUrl) {
          docs.medicalResults = { url: data.medicalResultsUrl, publicId: '', type: 'medicalResults', name: 'Medical Results' };
        }
        setDocuments(docs);
  ...
  const handleSubmit = async (status: 'DRAFT' | 'SUBMITTED') => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(status === 'DRAFT' ? 'Application updated successfully' : 'Application submitted successfully');
        router.push('/profile/applications');
      } else {
        const error = await response.json();
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-8">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-foreground truncate">Edit Application</h1>
            <p className="text-xs sm:text-sm text-muted-foreground truncate">Update your application details</p>
          </div>
