...
import { Globe, Calendar, Users, DollarSign, CheckCircle, Clock, XCircle, Eye } from 'lucide-react';
...
export default function VisaAssistancePage() {
  ...
  const [selectedBooking, setSelectedBooking] = useState<VisaBooking | null>(null);
  ...
  const fetchBookings = async () => {
    try {
      ...
      toast.error('Failed to load visa assistance bookings');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">
            Manage all visa assistance requests
          </p>
        </div>
        ...
        <div className="bg-card border-2 border-border rounded-xl overflow-hidden">
          {bookings.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-lg text-muted-foreground">No visa assistance bookings yet</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-foreground">Travel Date</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-foreground">Status</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-foreground">Fee</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-foreground">{booking.destinationCountry}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-foreground">{booking.travelPurpose}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Users className="w-4 h-4 text-muted-foreground" />
                          <span className="text-foreground">{booking.applicantCount}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-foreground">
                        {booking.plannedTravelDate 
                          ? new Date(booking.plannedTravelDate).toLocaleDateString()
                          : 'Not specified'}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-foreground">
                        {booking.finalFee ? `$${booking.finalFee}` : booking.feeEstimate ? `~$${booking.feeEstimate}` : 'TBD'}
                      </td>
                      <td className="px-6 py-4">
                        <button
      ...
      {selectedBooking && (
        ...
        >
          ...
          >
            <h2 className="text-2xl font-bold text-foreground mb-4">Booking Details</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-medium text-foreground">{selectedBooking.destinationCountry}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Travel Purpose</p>
                <p className="text-lg font-medium text-foreground">{selectedBooking.travelPurpose}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Number of Applicants</p>
                <p className="text-lg font-medium text-foreground">{selectedBooking.applicantCount}</p>
              </div>
            </div>
            <button
              onClick={() => setSelectedBooking(null)}
